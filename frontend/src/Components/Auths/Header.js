...
const Header = () => {
   const dispatch = useDispatch(); 
    ...
    const sendLogoutReq = async () => {
      const res = await axios.post("http://localhost:5000/api/v1/logout", null, {
        withCredentials: true
      });
      if (res.status === 200) {
        return res;
      }
      return new Error("Ba Zaka Iya Fitaba. Ka Kara Gwadawa");
    };
    const handleLogout = () => {
  ...
  return (
    <div>
      <AppBar position="sticky" sx={{bgcolor: "green"}} >
          <Toolbar>
              <Typography variant="h3">Kula Da Kudinka</Typography>
